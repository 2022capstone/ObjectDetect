...
def match_rent_id():
    ...
    for i in range(len(rent_ids_lists)):
        if (i == len(rent_ids_lists)):
            ...
            print("%d, rent_ids_lists :", i, rent_ids_lists[i])
            # 딥러닝 수행 및 데베 업데이트
    ...
    for data in results_data:
        ...
        for key, values in data.items():
            if db_column_idx < 1:
                ...
                # 데이터베이스에 업데이트
                # 이미지 path
                sql = "update rent_compare_img set " + db_column[db_column_idx] + " = '" + save_dir_img_path[
                    0] + "' where Rent_rent_id = " + str(rent_id_data) + ";"
                print(sql)
                cursor.execute(sql)
                # # 스크래치 갯수
                sql2 = "update rent_scratch_count set " + db_column_s[db_column_idx] + " = '" + str(
